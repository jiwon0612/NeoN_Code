...
namespace Work.JW.Code.FSM.Editor
{
    [CustomEditor(typeof(EntityStateDataSO))]
    public class StateDataEditor : UnityEditor.Editor
    {
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            inspectorUI.CloneTree(root); //ui를 복제해서 root의 자식으로 넣어줘
        ...
        private void CreateDropdownChoices(DropdownField dropdown)
        {
            ...
            List<string> typeList = fsmAssembly.GetTypes().Where(type =>
                type.IsAbstract == false && type.IsSubclassOf(typeof(EntityState))).Select(type => type.FullName).ToList();
